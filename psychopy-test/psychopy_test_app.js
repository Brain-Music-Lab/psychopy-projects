...
function button_press_taskRoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of button_press_taskComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function text_entry_taskRoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of text_entry_taskComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function slider_taskRoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of slider_taskComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
